...
const settings = ref({
  ...
  opacity: 0.95,
  scale: 1.0,
  redline: 2500, // 🏎️ DYNAMIC SHIFT CALIBRATION
  ...
  // Modules
  showDamage: 'SIMPLE', // OFF, SIMPLE, or ADVANCED
  showEcoDash: false,
  showWarnLights: true,
  showWaterTemp: true,
  showRpmText: true,
...
watch(() => truck.value.sdkActive, async (isActive) => {
  if (windowLabel === 'main') {
    if (isActive) await appWindow.show();
...
// Consumption: L/10km vs MPG
const displayCons = computed(() => {
  if (useMph.value) {
    // Convert L/km to MPG (approx)
...
<template>
  <div v-if="windowLabel === 'main'" class="hud-wrapper" :style="hudStyles" :class="{ 'hud-hidden': !truck.sdkActive }">
    ...
    <div class="warning-cluster" v-if="settings.showWarnLights">
      ...
      <div class="warn-icon" :class="{'warn-active warn-red': truck.battVoltWarning}">🔋</div>
    </div>
      ...
      <div class="hud-layout">
        ...
        <div class="hud-zone center-core">
          ...
          <div class="gear-module" v-if="settings.showGear">
            <span class="gear-value" :class="{ 'text-neutral': truck.gear === 0, 'text-reverse': truck.gear < 0 }">
              {{ gearDisplay }}
            </span>
          </div>
        </div>
            ...
            <div class="nav-stack">
              ...
              <template v-else>
                ...
                <div class="stat-row justify-end" v-if="settings.showCruise">
                  ...
                  <span class="value md" :class="truck.cruiseControl > 0 ? 'accent' : 'lcd-dimmed'">
                    {{ truck.cruiseControl > 0 ? Math.round(truck.cruiseControl * speedMult) : 'OFF' }}
                  </span>
                </div>
              </template>
            </div>
...
/* 🚨 Warning Cluster */
.warning-cluster { display: flex; gap: 15px; margin-bottom: 10px; background: rgba(0,0,0,0.6); padding: 5px 20px; border-radius: 20px; border: 1px solid rgba(255,255,255,0.05); box-shadow: 0 5px 15px rgba(0,0,0,0.5); }
.warn-icon { font-size: 18px; opacity: 0.1; filter: grayscale(100%); transition: 0.2s; }
...
.hud-container { width: auto; min-width: 1000px; height: 110px; background: linear-gradient(to bottom, rgba(10, 12, 16, var(--bg-opacity)), rgba(6, 8, 12, var(--bg-opacity))), repeating-linear-gradient(45deg, rgba(255,255,255,0.015) 0px, rgba(255,255,255,0.015) 2px, transparent 2px, transparent 4px); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border-bottom: 2px solid rgba(255, 255, 255, 0.05); box-shadow: 0 20px 50px rgba(0, 0, 0, 0.9), inset 0 1px 0 rgba(255, 255, 255, 0.1); display: flex; position: relative; pointer-events: none !important; transition: transform 0.6s cubic-bezier(0.16, 1, 0.3, 1); border-radius: 8px; }
.boot-sequence { opacity: 0; transform: translateY(40px) scale(0.95); }

.integrated-shift-bar { position: absolute; top: 0; left: 0; right: 0; height: 6px; display: flex; gap: 2px; background: #000; padding: 0 10%; border-radius: 8px 8px 0 0; overflow: hidden;}
...
.drag-handle { width: 30px; height: 100%; display: flex; align-items: center; justify-content: center; pointer-events: auto !important; cursor: grab; border-right: 1px solid rgba(255, 255, 255, 0.05); transition: 0.3s; border-radius: 8px 0 0 8px;}
...
.resize-handle { position: absolute; bottom: -8px; right: -8px; width: 30px; height: 30px; color: var(--brand-color); font-size: 20px; font-weight: bold; pointer-events: auto !important; cursor: nwse-resize; display: flex; justify-content: center; align-items: center; text-shadow: 0 0 10px var(--brand-glow); transition: 0.2s;}
...
.inner-grid { display: flex; flex-direction: column; gap: 2px; }
.inner-grid-right { display: flex; gap: 30px; align-items: center; }
...
.cmd-btn:hover { background: #2a2d3b; border-color: #555; }
